...
/**
 * 시퀀스에서 킥 목록을 뽑는다 — **저술이 지정한 캐리어**가 **공을 남에게 보내는** 순간만.
 *
 * 예전에는 "구간 시작 볼에서 가장 가까운 아무나"였다. 실측에서 그 선수는 수렴 로직에
 * 빨려온 일반 선수였고, 그래서 "패스가 선수를 거치지도 않고 휘어진다"가 됐다.
 *
 * ★ 2026-08-01(7라운드 ①) **드리블 터치를 킥에서 뺐다.**
 *   사용자 지적: "슛이나 패스를 실제로 하지 않는데 공을 가져가면서 슛·패스 모션을 계속 한다".
 *
 *   원인은 여기였다. 이 함수는 "캐리어가 있고 공이 1 m 이상 움직였다"만 보고 킥을 냈는데,
 *   `scenes.timeline`은 **같은 선수가 연속 소유하는 구간(=드리블)** 도 스테이션마다 키프레임을
 *   낸다(`dribble` 판정, scenes.ts). 그래서 몰고 가는 매 스테이션이 킥 한 번이 됐다.
 *
 *   실측(개편 전, 전 조합 4,512킥): **28.5%가 드리블 터치**였고 장면의 83.4%가 한 번 이상
 *   포함했다. 드리블 돌파 루트(`goal.e`)에서는 슈터가 공을 3.2~3.6 s 소유하는 동안
 *   **1.53 s(43~48%)가 킥 포즈**였고, 600 ms짜리 풀스윙이 세 번 연속 재생됐다 —
 *   화면에서는 정확히 "몰고 가면서 계속 발을 휘두른다"로 보인다.
 *
 *   처방: 다음 스텝의 소유자가 **나 자신이면 킥이 아니다**. 공은 계속 내 발 앞에 있고
 *   (`anchorPoint`가 캐리어 발에 맨다) 포즈는 달리기가 된다. 실제 축구의 드리블 터치는
 *   백스윙 260 ms · 팔로스루 340 ms짜리 동작이 아니라 보폭 안에서 끝나는 밀기이므로,
 *   달리기 보폭으로 표현하는 편이 오히려 정확하다.
 *
 *   `KICK_MIN_DISTANCE`로는 이것을 걸러낼 수 없었다 — 드리블 한 스테이션은 6~15 m를
 *   전진하므로 거리 문턱을 한참 넘는다. 걸러야 할 축은 거리가 아니라 **소유권 이전**이다.
 */
export function kickEvents(sequence: ChoreoStep[]): KickEvent[] {
  ...
  for (let k = 0; k + 1 < sequence.length; k++) {
    ...
    // 소유권이 넘어가지 않으면 킥이 아니다(드리블 터치·컨트롤 정지).
    if (sequence[k + 1].carrier === a.carrier) continue
