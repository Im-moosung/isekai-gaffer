...
export function buildFlowSequence(
  ...
  for (let k = 0; k < chain.length; k++) {
    ...
    seq.push({
      t: ms / FLOW_DWELL_MS,
      ball: { x: bx, y: by },
      // ★ 소유자를 명시한다. 무버를 내지 않아도 캐리어는 **실제 라인업 선수**이므로,
      //   3D 무브먼트가 공을 그 선수의 발에 앵커링하고 킥 모션도 그가 받는다.
      //   예전에는 carrier가 없어 무사건 분에는 킥이 한 번도 발생하지 않았고, 공은
      //   포메이션 좌표 사이를 아무도 차지 않은 채 미끄러졌다(실측: 최근접 선수 p50 2.77 m).
      carrier: holder.id,
