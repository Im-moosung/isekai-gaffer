...
const BASE: Record<MatchEventType, Tpl[]> = {
  ...
  shot: pool('shot', 2, [
    ...
    // ★ 발화만 `슛이`를 넣는다. `{P}, 발끝을 떠납니다!`는 **뜻이 바뀐다** — 원문은
    //   *공이* 발끝을 떠나는 것인데, 쉼표 동격으로 접으면 선수가 (경기장을) 떠나는
    //   것으로 들린다. miss.1의 `{P}, 슛이 골문을 벗어납니다.`와 같은 해법이다.
    [v => `${v.player}의 발끝을 떠납니다!`, v => `${v.player}, 슛이 발끝을 떠납니다!`],
  ]),
...
const AN_BASE: Partial<Record<MatchEventType, Tpl[]>> = {
  ...
  save: pool('an.save', 1, [
    () => '네, 이런 선방이 승점을 지킵니다.',
    // ★ 이름을 뺐다. 실측(tools/tts/roster.mjs)에서 **해설위원이 이름을 부르는
    //   템플릿은 이것 하나뿐**이었고(이름 든 문장의 4.1%), 그 이름은 언제나
    //   선방한 골키퍼였다. 이름을 `골키퍼`로 바꾸면 뜻이 그대로인 채
    //   해설위원용 이름 클립이 12개(팀당 GK 1명) → **0개**가 된다.
    //   해설위원 모델의 RPD 10을 통째로 아끼는 한 줄이다.
    () => '골키퍼, 자리 선정이 좋았습니다.',
    () => '각을 좁힌 게 컸어요.',
    ...
    if (key === 'GK') {
      ...
      for (const m of group) {
        push({
          ...
          // ★ `골키퍼, 김승규.`가 아니라 `…입니다.`로 닫는다. 아래 그룹의 마지막
          //   호명(`{이름}입니다.`)과 **같은 형태**가 되어, TTS mp3가 이름 조각을
          //   두 형태(`{이름},` / `{이름}입니다.`)만 구우면 되게 만든다.
          //   `골키퍼,` + `{이름}` + `입니다.` 3조각으로 쪼개면 이음매가 `범+입`처럼
          //   음절 한복판에 떨어져 연음이 깨진다(docs/audio/tts/README.md).
          text: `골키퍼 ${m.nameKo}`, speech: `골키퍼, ${m.nameKo}입니다.`,
        })
      }
      continue
    }
