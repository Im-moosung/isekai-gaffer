...
describe('공격 패턴 축 (P3) — 크로스와 중앙 침투의 서열이 상대에 따라 뒤집혀야 한다', () => {
  ...
  // n=4000 실측(상성표 재설계 + 4-3-3↔4-2-3-1 정정 후 전수 재측정, 2026-08-01):
  //   rsa +0.186/+0.201 · cze +0.131/+0.149 · mex −0.043/−0.031 ·
  //   esp −0.129/−0.126 · fra −0.123/−0.132
  // ⚠ fra는 이 축의 최약 지점이다. 한국 기본 4-2-3-1 vs 프랑스 4-3-3의 상성값이 곧
  //   기울기로 새기 때문이다(형태 우위 → 상대 진영에서의 여유 → 크로스·침투의 차이 압축).
  //   재설계 첫 판이 이 쌍을 +0.04(한국 유리)로 뒀을 때 −0.066/−0.056까지 얕아져 이 게이트가
  //   깨졌고, 쌍을 −0.02로 정정해 복구했다(engine/tactics.ts EDGES 주석·리서치 §3.1a).
  //   EDGES를 손대면 이 다섯 줄을 반드시 다시 재라 — 상성표의 부작용이 가장 먼저 보이는 곳이다.
  const N_AP = 4000
