...
// 미리 구워 둔 중계 mp3 재생. `commentary-tts.ts`의 **유일한** 경로다 —
// 여기서 못 내면 그 문장은 **소리가 나지 않는다**(2026-08-02, `speechSynthesis` 폴백 제거).
// 브라우저 기본 음성은 여성이라 남성 캐스터 사이에 한 문장만 끼어들면 결함으로 읽힌다.
// 자막은 폴백과 무관하게 나가므로 침묵이 낫다는 판단이다 — 근거는 commentary-tts.ts 헤더.
//
// ★ 그래서 이 모듈의 실패는 이제 **들리지 않는 실패**다. "왜 저 문장만 조용하지"를
//   추적할 수 있게, 못 내는 경로마다 이유를 주석으로 남긴다(조회표 구멍 / 버퍼 미도착).
//
// 핵심 원칙(불변):
...
//
// ★ 덮이지 않는 문장은 **그 문장만** 조용하다(대본과 다르다 — `canSpeakAll` 참조).
//   경기 중 문장은 서로 독립이라 한 줄이 빠져도 다음 줄에서 회복된다.
//   그래도 구멍은 구멍이므로 커버리지 100%가 목표다(`__tests__/tts-coverage.test.ts`).
 ...
 * ★ 입장 소개는 전부 아니면 전무다. 스물 몇 줄 중 몇 줄만 소리가 나면 명단에서 몇 명을
 *   빠뜨린 것처럼 들린다 — 통째로 조용한 편이 낫다(commentary-tts.beginScript).
 */
 ...
 *   그 줄은 소리가 나지 않는다. 경기 중 조각은 800개가 넘어 그냥 두면 **모든 조각의
 *   첫 등장이 무음**이 된다. 입장 연출은 13~63초짜리라 그 사이에 받아 두면 킥오프
 *   시점엔 대부분 준비된다.
 *
 * 순서는 조회표의 `warm`(코퍼스 빈도 내림차순)을 그대로 따른다 — 늦게 도착해도
 * **자주 쓰는 것부터** 준비된다. 실패는 조용히 넘긴다(그 조각만 무음).
 *
 * ★ **`warm`에 없는 클립도 꼬리에 붙여 전부 받는다**(2026-08-02). `warm`은 굽기
 *   도구가 그날의 코퍼스 계획(`tools/tts/live-package.py`의 `plan`)으로 만드는데,
 *   계획 뒤에 손으로 추가한 조각은 `clips`에만 들어가고 `warm`에서 빠진다. 실제로
 *   1,024개 중 140개가 그랬고, 그중 하나가 `여기서, 또`였다 — 사용자가 여성 음성으로
 *   들었다고 보고한 "여기서, 또 김태현, 반칙이 잦네요."의 첫 조각이다. 조회표에는
 *   있는데 미리 받지 않으니 **첫 등장에서 언제나 폴백**했고, 그런 문장은 대개 한 경기에
 *   한 번뿐이라 사실상 영구 폴백이었다. 폴백이 사라진 지금 그대로 두면 같은 문장이
 *   **영구 무음**이 된다. 조회표를 진실원으로 삼아 런타임에서 메운다 — 굽기 도구를
 *   다시 돌리지 않아도 되고, 앞으로 조각을 더 추가해도 같은 구멍이 생기지 않는다.
 *   비용은 꼬리에 붙는 1.7MB뿐이다(빈도 순 9.4MB를 다 받은 **뒤**에 온다).
 */
export function warmLive(): void {
  if (warmed || !index) return
  const bus = audioBus()
  if (!bus) return // 아직 유저 제스처 전 — 다음 호출에서 다시 시도한다.
  warmed = true
  // 빈도 순(`warm`)이 앞, 계획에서 빠진 나머지가 뒤. 중복은 첫 등장만 남긴다.
  const seen = new Set<string>()
  const queue: string[] = []
  for (const k of [...(index.warm ?? []), ...Object.values(index.clips)]) {
...
/**
 * 한 줄을 mp3로 낸다. **동기적으로 true/false를 돌려준다** — 발화 시각이 정해져 있어
 * 호출부가 그 자리에서 결과를 알아야 하기 때문이다. 그래서 판정은 *조회표 + 이미
 * 디코드된 버퍼*로만 한다. false가 나오는 경우는 둘뿐이다:
 *   ① 조회표가 이 문장을 못 덮는다(커버리지 구멍) → 그 문장은 **영구 무음**이다.
 *   ② 클립은 있는데 버퍼가 아직 안 왔다 → 이번엔 무음, 백그라운드로 받아 둔다.
 * ②는 {@link warmLive}가 입장 연출 동안 전부 받아 두어 거의 일어나지 않게 했다 —
 * 폴백이 없어진 뒤로 ②는 "한 번쯤 목소리가 달랐다"가 아니라 "그 문장이 통째로
 * 사라졌다"가 되기 때문이다.
 */
