...
## 음성 — Gemini TTS 생성 오디오 (docs/audio/tts-proto/, 향후 public/voice/)

| 파일 | 용도 | 생성 도구 | 출처 URL | 라이선스 | 확인일 |
|------|------|----------|----------|----------|--------|
| `docs/audio/tts-proto/*.mp3` | 중계 음성 이음매 검증 프로토타입 | Google Gemini API TTS — 캐스터 `gemini-2.5-flash-preview-tts`(voice **Puck**) / 해설위원 `gemini-3.1-flash-tts-preview`(voice **Iapetus**) | https://ai.google.dev/gemini-api/terms | 생성물 소유권 **미주장**(아래 인용) — 배포·상업 이용 가능 | 2026-08-01 |

### 근거 (원문 인용)
- 소유권 — Gemini API Additional Terms of Service (Effective March 23, 2026),
...
- preview 모델의 프로덕션 사용 — https://ai.google.dev/gemini-api/docs/models
  > "**Preview** — Points to a preview model which **may be used for production**. ... will be deprecated with at least 2 weeks notice."
  > "**Experimental** — ... will typically **not be suitable for production use**."

  → 우리가 쓰는 두 모델은 모두 `preview` 계열이라 프로덕션 사용이 문서상 명시적으로 허용된다.
  금지 대상은 `-exp` 접미사의 experimental 계열이다. **다만 preview는 2주 예고 후 폐기될 수 있으므로,
  mp3를 빌드 타임에 굽고 저장소에 커밋해 두어야 한다** — 런타임 호출은 모델 폐기와 함께 죽는다.
- 무료 등급의 데이터 취급 — 같은 문서
  ...
  → 중계 대본에 비공개 정보가 없어 실무상 무해하다. **무료 등급에 상업 이용을 금지하는 조항은 약관 전문에서 찾지 못했다.**
- 사칭 금지 — Generative AI Prohibited Use Policy, https://policies.google.com/terms/generative-ai/use-policy
  ...
  → **실존 캐스터·해설위원의 이름이나 특징을 프롬프트에 넣지 않는다.** 현재 프롬프트는
  "축구 중계 캐스터의 목소리로" 같은 가상 페르소나뿐이다.
- 워터마크 — https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-3-1-flash-tts/
  > "**All audio generated by Gemini 3.1 Flash TTS is watermarked with SynthID.** This imperceptible watermark is interwoven directly into the audio output."

  → 사용자 의무가 아니라 Google이 자동 삽입한다. 제거하지 않는다.

### 확인했으나 **찾지 못한** 것 (추측하지 않는다)
- prebuilt 음성(Puck·Iapetus) 자체에 대한 **별도 라이선스·성우 크레딧·표기 의무 조항**.
  https://ai.google.dev/gemini-api/docs/speech-generation 에 음성 목록만 있고 라이선스 문구가 없다.
  Google APIs ToS §6(b)가 "documentation에 기술된 attribution을 표시하라"고 하는데,
  speech-generation 문서에는 요구되는 attribution이 명시돼 있지 않다.
- `gemini-2.5-flash-preview-tts`(캐스터 모델)의 **SynthID 적용 여부를 명시한 공식 문서**. 3.1만 확인된다.
- 생성 출력물이 Google APIs ToS §5(e)(https://developers.google.com/terms)의
  "distribute, publicly display" 금지의 **예외라고 명시한 문장**. §5(e) 서두의
  "Unless expressly permitted by the content owner"와 위 소유권 조항이 결합해
  생성물에는 적용되지 않는다고 읽히지만, 명문 규정은 없다.

### 이 프로젝트에 실제로 걸리는 조항과 대응
- **연령** — "You also will not use the Services as part of a ... service that is directed towards or is likely to be accessed by individuals under the age of 18."
- **지역** — "You may use only Paid Services when making API Clients available to users in the European Economic Area, Switzerland, or the United Kingdom."

  → 두 조항 모두 배포물이 **API Client**일 때 걸린다. **mp3를 빌드 타임에 사전 생성해
  정적 자산으로 커밋하고 런타임에 Gemini를 호출하지 않으면** 배포되는 웹서비스는
  API Client가 아니다. 현재 설계가 그렇다. **런타임 TTS 호출은 도입하지 않는다.**
- 크레딧에 "중계 음성은 Google Gemini TTS로 생성한 AI 음성" 한 줄을 넣는다. 법적 의무는
  아니지만 위 사칭 금지 조항 대응상 무해하고 심사 신뢰도에도 유리하다.
