...
- **코치 회의 (멀티 코치) ★사용자 확정**: 브레이크·HT 진입 시 `coach.ts`가 **코치진 3~4명의 서로 다른 관점 제안** 생성 — 수비 코치(실점 존·라인 분석), 공격 코치(찬스 패턴·공격 방향), 피지컬 코치(체력 하위 선수·압박 지속 경고), 세트피스 코치(코너·키커 — 해당 상황 시). 각 카드: 코치 직함(익명 — 실명 금지) + 근거 데이터 + 제안 지시 + **[채택]** 원클릭. 카드 목록 하단에 항상 **[감독 판단대로 간다]**(전부 무시) — 유저가 어느 의견을 믿을지 스스로 결정. 제안 간 상충 허용(수비 코치 "내려라" vs 공격 코치 "올려라" — 감독의 딜레마). 결정론·실측 기반·비하 금지
...
**계약:** 기존 계획(사기 게이지 사전 표시·선수별 반응 아이콘 2~3명) + **사용자 확정 확장**:
- **문장형 선택지**: "격려 — {문장}" 형식. 문장은 docs/research/team-talk-styles.md의 24문장 세트(톤 4×상황 3×변형 2 — 유명 감독 스타일 영감 창작, 실명 미노출) 사용. 상황(스코어)에 맞는 톤별 문장이 자동 선택되어 4버튼에 표시(변형은 경기 시드 결정론)
- **선택 즉시 효과 표시 ★**: 클릭 직후 큰 피드백 배너 — "선수단 사기 상승 (+8)" / "역효과! 선수단 사기 저하 (-12)" + 선수별 반응 아이콘. applyTeamTalk delta 반환 기반
- 코치 추천 뱃지는 유지하되 **미묘하게**(정답 노출이 아니라 힌트 — 추천이 틀릴 수도 있는 확률 요소는 넣지 않음, 결정론 유지)
- FM 확장: ① 같은 톤 반복 반감(campaignStore.lastTeamTalkTone) ② 상대 기대치(FIFA 랭킹 차) 보정 테이블
