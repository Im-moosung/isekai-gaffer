...
| 실패 게이트 | 조정할 계수 | 방향 |
|---|---|---|
| 라인 최적값이 상대별로 안 갈림 | `suppression`의 `possFactor` 폭(0.7~1.3) | 넓힌다 |
| 압박 최적값이 상대별로 안 갈림 | `gkFactor` 범위(0.35~1.4) | 넓힌다 |
| 라인 승점 차 > 0.30 | `suppression`의 0.12 계수 | 여전히 단조 감소면 올리고, 단조 증가로 뒤집혔으면 내린다 |
| 압박 승점 차 > 0.30 | `pressGain`의 0.15 계수 / 지속압박 임계 | 임계 70→75로 올린다(`simulate.ts:138,141,175`) |
