#!/usr/bin/env node
// tools/sim-audit/shot-live.mjs
// R5 전용 **실주행 연속 캡처** — 킥 임팩트 창을 촘촘히 훑는다. 실제 Chrome(headless:false)을 몰아
...
try {
  ...
  if (PATTERN) {
    // 워룸에서 공격 패턴을 고른다 — 이것이 화면에 보이는지가 이번 캡처의 논점이다.
    // 탭이 있으면 먼저 연다(워룸은 라인업/팀 전술/상대 브리핑 3탭이다).
    for (const name of ['팀 전술', '전술']) {
      ...
      if (clicked) { await page.waitForTimeout(400); break }
    }
    console.log('  워룸 버튼:', await page.evaluate(() => [...document.querySelectorAll('button')].map(b => b.textContent?.trim()).join(' | ')))
    // 탭 패널이 display:none으로 남아 있어도 상태는 같은 store다 — 보이지 않아도 누른다.
    ...
    if (!ok) throw new Error(`워룸에서 '${PATTERN}' 버튼을 찾지 못했다`)
  ...
  while (want.length > 0 && Date.now() < deadline) {
    ...
    if (!SCENE_RE.test(info.scene)) { await page.waitForTimeout(90); continue }
